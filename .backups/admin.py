...
def list_users(
    ...
    Super admin видит всех, обычный admin - только из своей организации"""
    
    # Обновляем статус неактивных пользователей
    ...
    # Фильтрация по организации для обычного админа
    if current_user.role == UserRole.super_admin:
        users = db.query(User).order_by(User.id.asc()).all()
    else:
        # Обычный admin видит только пользователей своей организации
...
def list_users_full(
    ...
    """Return all users with full set of parameters. 
    Super admin видит всех, обычный admin - только из своей организации.
    By default sensitive fields like password_hash are omitted unless include_sensitive=true."""
...
# =====================================================
# 🔹 DATABASE ACCESS — управление доступом к БД
# =====================================================
...
def get_user_database_access(
    ...
    """Получить все доступы пользователя к базам данных"""
    target_user = db.query(User).get(user_id)
    if not target_user:
        raise HTTPException(status_code=404, detail="User not found")
    
    # Проверка прав: admin может видеть только пользователей своей организации
...
def create_database_access(
    ...
    # Проверка существования пользователя и БД
    target_user = db.query(User).get(user_id)
    if not target_user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    # Проверка прав: admin работает только с пользователями и БД своей организации
    if current_user.role == UserRole.admin:
        if target_user.organization_id != current_user.organization_id:
            raise HTTPException(status_code=403, detail="Cannot grant access to users from other organizations")
        
        # Проверяем, что БД принадлежит клиенту из той же организации
...
def update_database_access(
    ...
    """Обновить права доступа к базе данных"""
    from models.user_database_access import UserDatabaseAccess
    ...
    # Проверка прав: admin может обновлять только доступы пользователей своей организации
    if current_user.role == UserRole.admin:
...
def delete_database_access(
    ...
    # Проверка прав: admin может удалять только доступы пользователей своей организации
    if current_user.role == UserRole.admin:
        target_user = db.query(User).get(access.user_id)
        if target_user.organization_id != current_user.organization_id:
            raise HTTPException(status_code=403, detail="Access denied")
